"""
FastAPI application for the real-time tank simulation system.

This service exposes:
- REST endpoints for initializing the real-time simulator
- WebSocket endpoint for streaming state updates and receiving setpoint changes
"""
...
from app.src.models.simulation_models import RealTimeConfig, SetpointCommand
...
app = FastAPI(
    ...
    description=(
        "API for simulating a 5-tank process control system in real-time via WebSocket"
    ),
    version="2.0.0",
    lifespan=lifespan,
)
...
async def get_synoptic() -> FileResponse:
    ...
    synoptic diagram and handles user interactions for the real-time mode.
    ...
    try:
        ...
        while True:
            ...
            if data.get("type") == "setpoint":
                ...
                logger.info(
                    f"Setpoint updated: {command.tank_id} -> {command.variable}"
                )
